    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            for t in trades:
                ...
                # Fixed Allocation Assessment based on Conviction
                if score >= 90:
                    allocation_percent = 0.15
                else:
                    allocation_percent = 0.10
                    
                capital_required = current_total_equity * allocation_percent
                
                if capital_required > current_available:
                    capital_required = max(0.0, current_available)
                    
                shares_to_buy = int(capital_required / ep)
                final_capital = shares_to_buy * ep
                ...
                cur.execute("""
                    ...
                    WHERE id = %s
                """, (float(final_capital), shares_to_buy, float(pnl_rs), tid))
