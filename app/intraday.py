# =====================================================================================
# app/intraday.py (SCHEDULER READY)
...
def start():
    ...
    if not is_market_regime_bullish():
        logger.info("🛑 Bearish macro regime detected. Skipping INTRADAY scan.")
        return
    # ────────────────────────────────────────────────────────────────────────
    ...
    logger.info("=" * 80)
    logger.info(f"⚡ INTRADAY SCAN START | {ist_now.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("=" * 80)
    ...
    try:
        ...
        for idx, (_, row) in enumerate(watchlist.iterrows(), start=1):
            ...
            try:
                ...
                metrics = {
                    "scan_type": "INTRADAY", "score": score, "rsi": round(rsi_val, 1),
