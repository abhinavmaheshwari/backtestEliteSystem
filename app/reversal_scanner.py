...
def start():
    ...
    try:
        ...
        for _, row in watchlist.iterrows():
            ...
            if symbol not in all_ticker_data or all_ticker_data[symbol].empty: continue
            
            ticker = all_ticker_data[symbol].copy()
            if isinstance(ticker.columns, pd.MultiIndex): ticker.columns = ticker.columns.get_level_values(0)
            ...
            if drop_pct < MIN_DROP_FROM_52W_HIGH or drop_pct > MAX_DROP_FROM_52W_HIGH: continue

            current_rsi = float(latest["RSI"])
            past_10_rsi = ticker["RSI"].iloc[-11:-1].min()
            
            if current_rsi < RSI_CURL_MIN or past_10_rsi > RSI_OVERSOLD_THRESHOLD: continue 
                
            ema20 = float(latest["EMA20"])
            if close_price < ema20: continue 
            ...
            if macd < macd_sig or macd > 2.0: continue
            ...
            metrics = {
                "scan_type": "REVERSAL", "score": 85, "rsi": round(current_rsi, 1),
                "volume_ratio": round(vol_ratio, 2), "delivery_pct": None,
                "peg": row.get("PEG Ratio"), "roe": row.get("ROE %"),
                "yoy_profit": row.get("YOY Profit %"), "atr_stop": round(suggested_stop, 2)
            }

            if not save_alert_if_new(symbol, dedup_key, ist_now.strftime("%Y-%m-%d %H:%M:%S"), metrics=metrics):
                continue

            alerts_by_category.setdefault(category, []).append({
                "symbol": symbol, "category": category, "breakout_signals": reversal_signals, 
                "price": round(close_price, 2), "open": round(float(latest["Open"]), 2),
                "day_high": round(float(latest["High"]), 2), "day_low": round(float(latest["Low"]), 2),
                "rsi": round(current_rsi, 1), "volume_ratio": round(vol_ratio, 2),
                "body_ratio": round(abs(close_price - float(latest["Open"])) / candle_range * 100) if candle_range > 0 else 0,
                "score": 85, "above_ema20": True, "atr_stop": round(suggested_stop, 2),
                "peg": row.get("PEG Ratio"), "yoy_rev": row.get("YOY Revenue %"),
                "yoy_profit": row.get("YOY Profit %"), "roe": row.get("ROE %")
            })
            total_alerts += 1
