# =====================================================================================
# app/live_scanner.py (SCHEDULER READY)
...
def start():
    ...
    if not is_market_regime_bullish():
        logger.info("🛑 Bearish macro regime detected. Skipping 1H scan.")
        return
    # ────────────────────────────────────────────────────────────────────────

    ist_now = datetime.now(IST)
    logger.info("=" * 80)
    logger.info(f"🚀 1H SCAN START | {ist_now.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("=" * 80)
    ...
    try:
        ...
        for idx, (_, row) in enumerate(watchlist.iterrows(), start=1):
            ...
            try:
                ...
                metrics = {
                    "scan_type": "1H", "score": score, "rsi": round(rsi_val, 1),
                ...
                above_ema20 = bool(float(latest["Close"]) >= float(latest["EMA20"])) if "EMA20" in ticker.columns else None
                above_sma50 = bool(float(latest["Close"]) >= float(latest["SMA50"])) if "SMA50" in ticker.columns else None
        ...
        logger.exception("❌ CRITICAL 1H SCAN ERROR")
